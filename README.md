...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-309%20frontend%20%7C%20129%20backend-176B55" alt="Test counts">
...
ALGET pairs canonical engineering and instructional-design content with a learning environment that responds to each learner — pace, confusions, strong concepts, weak ones — while staying grounded in source material through a debate loop of specialist agents and a peer-review validator. Instructor sources now follow one governed path: Google Docs, PDFs, and approved URLs become page-addressable evidence, a reviewable shadow draft, retrieval chunks, formative checks, and only then a human-approved learner module. It is the reference implementation for the system paper currently being prepared (`paper_draft.md`).

```mermaid
flowchart LR
    SRC["Instructor sources<br/>Google Docs · PDF · URL"] --> ING["Ingestion + quality<br/>checksum · page chunks · OCR flags"]
    ING --> DRAFT["Shadow draft<br/>reading · concepts · practice · KB"]
    DRAFT --> GATE["Human release gate<br/>review → ready → publish"]
    GATE --> P["Cloudflare Pages<br/>React 19 · published snapshots"]
    U["Learner · Instructor · Admin"] --> P
...
Real screenshots captured from the current app via Playwright on 2026-09-06 (1440×900 desktop, full-page dashboard views, and a live text-selection state). The canonical set is in [`screenshots/latest/`](screenshots/latest/) with capture metadata in [`manifest.json`](screenshots/latest/manifest.json); it was generated from commit `8f350f8` with the local FastAPI parity runtime enabled. Agentic examples contain no learner records.

<table>
<tr>
<td width="33%" align="center">
  <a href="screenshots/latest/03_book_reader.png"><img src="screenshots/latest/03_book_reader.png" alt="ALGET reader with course navigation, live presence, and an embedded decision activity"></a>
...
<td width="33%" align="center">
  <a href="screenshots/latest/06_student_dashboard.png"><img src="screenshots/latest/06_student_dashboard.png" alt="Learner dashboard showing an evidence-based weekly plan awaiting learner approval"></a>
  <br><sub><b>Learner-owned planner</b> — <code>/dashboard</code><br>mastery evidence → editable plan → explicit approval</sub>
</td>
<td width="33%" align="center">
  <a href="screenshots/latest/07_instructor_dashboard.png"><img src="screenshots/latest/07_instructor_dashboard.png" alt="Instructor intervention queue showing evidence, proposal, and human approval controls"></a>
  <br><sub><b>Instructor intervention queue</b> — <code>/instructor</code><br>evidence → bounded proposal → human decision</sub>
</td>
</tr>
<tr>
<td width="33%" align="center">
  <a href="screenshots/latest/02_course_chooser.png"><img src="screenshots/latest/02_course_chooser.png" alt="Course chooser at /learn"></a>
  <br><sub><b>Course pathways</b> — <code>/learn</code><br>eight engineering and education pathways</sub>
</td>
<td width="33%" align="center">
  <a href="screenshots/latest/09_highlight_popover.png"><img src="screenshots/latest/09_highlight_popover.png" alt="Highlight popover with Highlight, Add Note, Ask AI"></a>
  <br><sub><b>Social annotation</b><br>Highlight · Add Note · Ask AI · evidence trace</sub>
</td>
<td width="33%" align="center">
  <a href="screenshots/latest/08_analytics.png"><img src="screenshots/latest/08_analytics.png" alt="Research console"></a>
...
```mermaid
flowchart TB
    subgraph Sources["Faculty source pipeline · accountable authoring"]
      SRC["Google Docs · PDF · approved URL"]
      ING["Extraction + normalization<br/>checksum · page-addressable text"]
      DRAFT["Shadow draft<br/>reading · activity · simulation · KB · practice"]
      REVIEW["Instructor review<br/>quality flags · references · release gate"]
      SRC --> ING --> DRAFT --> REVIEW
    end

    subgraph Client["Cloudflare Pages · browser"]
      UI["React learning workspace"]
      C["256 static MDX-derived content snapshots"]
      PUB["Published instructor modules<br/>retrieval context + formative checks"]
...
|---|---|---|
| **Faculty source pipeline** | Google Docs/PDF/URL extraction, checksums, page chunks, deterministic source-grounded draft, quality flags, and human release gate | Unreviewed or weak sources stay in shadow mode and never become learner-visible. |
| **Cloudflare Pages** | SPA, fonts, diagrams, reference images, immutable course JSON, and approved module snapshots | Core reading remains available without an AI response. |
...
| `/api/faculty/google-docs/import` | POST | Authenticated instructor source import → shadow draft with reading, concepts, KB chunks, practice, tutor, analytics, and social contracts |
| `/api/faculty/pdf/import` | POST | Authenticated instructor PDF extraction → the same shadow-draft contract with page metadata and checksum |
| `/api/admin/pdf/convert` | POST | Admin PDF conversion and page-addressable source quality report |
...
│
├── screenshots/                       ← Playwright captures; `latest/` is the current release set
...
| `docs/ALGET_SOURCE_PIPELINE.md` | maintainers / instructors | Current Google Docs/PDF/URL → shadow draft → retrieval-grounded module architecture |
