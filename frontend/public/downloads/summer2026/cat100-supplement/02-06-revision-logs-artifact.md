...
## Purpose
This packet turns **Revision Logs** into a concrete artifact studio task. The goal is not to make a polished submission on the first pass. The goal is to make your judgment visible enough that an instructor, peer, or future version of you can review the decision.
...
Strong trace: "I asked for three alternatives for the revision log. I accepted the alternative that made the audience constraint explicit, rejected the alternative that removed the evidence source, and revised the artifact so the reader can see how NACE career readiness shaped the final decision. The remaining limitation is that the trace has not yet been tested with a second reader."
