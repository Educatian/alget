...
test('Google Docs course drafting keeps generated experiences in shadow review', () => {
  ...
  assert.ok(draft.sections[0].reading.content)
  assert.ok(draft.sections[0].practice.problems.length >= 2)
  assert.ok(draft.sections[0].knowledge_base.chunks.length >= 1)
