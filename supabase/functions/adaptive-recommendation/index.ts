// ---------------------------------------------------------------------------
// Supabase Edge Function: adaptive-recommendation
//
// Low-latency, co-located port of the FastAPI /adaptive_recommendation pure
...
interface AdaptiveRequest {
  ...
  artifact_revision_scores?: any[];
  annotation_signals?: any[];
...
function assembleFeatures(req: AdaptiveRequest): FeatureVector {
  ...
  if (artifactScores.length > 0) {
    ...
    for (const s of artifactScores) {
      const q = num((s as any)?.quality ?? (s as any)?.after ?? (s as any)?.score);
      qSum += q;
      dSum += num((s as any)?.delta ?? 0);
    }
  ...
  if (annotationSignals.length > 0) {
    ...
    for (const a of annotationSignals) {
      ...
      if ((a as any)?.section_overlap || (a as any)?.same_section) {
        overlapCount += 1;
      }
    }
...
// the caller: a backend failure must not break the live recommendation.
async function persistDecision(record: Record<string, unknown>): Promise<boolean> {
...
async function handle(request: Request): Promise<Response> {
  ...
  const decision = select_support_move(features, annotationAdaptive, preferAdvance);
  ...
  // Best-effort persistence; never blocks/throws to the caller.
  const persisted = await persistDecision(record);
...
export { assembleFeatures, handle };
