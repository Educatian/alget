...
- [ ] A live authenticated PDF/Google Docs smoke still requires a designated instructor pilot token; no test identity or credential is stored in the repository or CI. Use `scripts/faculty_authenticated_smoke.mjs` with a short-lived `ALGET_INSTRUCTOR_TOKEN` and source-specific environment variables.
